...
st.set_page_config(page_title="Nayara - Simulador Térmico v1.6.6", layout="wide")
...
st.sidebar.subheader("🏗️ Materiais")
...
def ler_shapefile_zip(uploaded_file):
    with tempfile.TemporaryDirectory() as tmpdir:
        path_zip = os.path.join(tmpdir, "data.zip")
        ...
        # Procura o arquivo .shp (ignora pastas __MACOSX se houver)
        for root, dirs, files in os.walk(tmpdir):
            if "__MACOSX" in root: continue
            for file in files:
                if file.endswith(".shp"):
                    # Força o uso do engine 'pyogrio' que é mais estável
                    return gpd.read_file(os.path.join(root, file), engine='pyogrio')
    return None

# --- 1. CONFIGURAÇÃO DAS BASES ---
...
if geo_file:
    ...
    try:
        gdf_limite = ler_shapefile_zip(geo_file)
        if gdf_limite is not None:
            # Converte para WGS84 para o Mapbox
            ...
            if bld_file:
                gdf_bld = ler_shapefile_zip(bld_file)
                if gdf_bld is not None:
                    if gdf_bld.crs != "EPSG:4326": gdf_bld = gdf_bld.to_crs("EPSG:4326")
...
if btn_simular:
    ...
    with st.expander("📖 Notas Científicas e Metodologia Aplicada"):
        st.write(f"Simulação baseada em Fortaleza. Emissividade: {emissividade} ({material}).")
