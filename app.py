...
if pagina == "Sobre":
    ...
    Esta plataforma foi desenvolvida como parte de uma pesquisa de doutorado para simular o comportamento térmico de superfícies urbanas, focando no microclima de **Fortaleza/CE**.
    
    O modelo utiliza equações de balanço de energia e inércia térmica para prever como diferentes materiais e configurações urbanas (sombra, água, vegetação) influenciam a temperatura superficial e subsuperficial ao longo de 24 horas.
    
    ### Artigos Relacionados e Referências
    * **Células Urbanas e Microclima:** Estudos sobre o impacto do albedo e emissividade em materiais de pavimentação.
    * **Simulação Paramétrica:** Baseado em algoritmos simplificados de motores de cálculo como o ENVI-met.
    * **Termodinâmica do Solo:** Modelagem de fluxo de calor condutivo para a camada de 5cm.
    """)
    
    st.info("No futuro, você pode listar aqui os links diretos para seus artigos publicados.")

# ==========================================
# PÁGINA: SIMULE SUA ÁREA DE ESTUDO
# ==========================================
else:
    ...
    if btn_simular:
        ...
        st.session_state['resultados'] = {
            ...
            'albedo': albedo,
            'taxa_sombra': taxa_sombra,
            'taxa_edificada': taxa_edificada,
            'taxa_agua': taxa_agua,
            'taxa_permeavel': taxa_permeavel
        }
    ...
    if 'resultados' in st.session_state:
        ...
        with col_stats:
            ...
            with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
                ...
                writer.sheets['Simulacao'].set_column('A:C', 15)
                
            st.download_button("📥 Baixar Planilha Excel", output.getvalue(), f"simulacao_{res['material']}.xlsx", use_container_width=True)

        # NOTAS CIENTÍFICAS COMPLETAS (Restauradas e preservadas)
        with st.expander("📖 Notas Científicas e Metodologia Aplicada"):
            ...
            1. **Balanço de Energia:** Considera a Radiação Solar Incidente filtrada pelo sombreamento ({res['taxa_sombra']}%) e área edificada ({res['taxa_edificada']}%).
            2. **Materiais:** Utiliza emissividade de **{res['emissividade']}** para o material **{res['material']}** (conforme dados corrigidos do usuário).
            ...
            4. **Mitigação Evaporativa:** O efeito de corpos d'água ({res['taxa_agua']}%) e solo permeável ({res['taxa_permeavel']}%) contribui para a redução do calor sensível.
