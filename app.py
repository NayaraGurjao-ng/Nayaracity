...
st.set_page_config(page_title="Nayara - Simulador Térmico v1.6.7", layout="wide")
...
def ler_mapa_zip(uploaded_file):
    try:
        with tempfile.TemporaryDirectory() as tmpdir:
            path_zip = os.path.join(tmpdir, "upload.zip")
            ...
            # Busca o arquivo .shp ignorando pastas de sistema
            for root, dirs, files in os.walk(tmpdir):
                if "__MACOSX" in root: continue
                for file in files:
                    if file.endswith(".shp"):
                        # Engine pyogrio é fundamental para o Streamlit Cloud
                        gdf = gpd.read_file(os.path.join(root, file), engine='pyogrio')
                        # Garante conversão para coordenadas geográficas (Mapbox)
...
# Lógica visual do quadrado de pixels
...
vazios = np.where(mapa_data.flat == 0)[0]
idx_verde = np.random.choice(vazios, min(len(vazios), int(((taxa_sombra+taxa_permeavel)/2/100)*grid_dim**2)), replace=False)
mapa_data.flat[idx_verde] = 1

fig_teorico = px.imshow(mapa_data, color_continuous_scale=['#444444', '#228B22', '#8B4513', '#1E90FF'])
...
if geo_file:
    ...
    gdf_limite = ler_mapa_zip(geo_file)
...
if btn_simular:
    ...
    with col_g:
        ...
        fig_res.update_layout(xaxis_title="Hora", yaxis_title="Temp (°C)", legend=dict(orientation="h", y=1.1))
    ...
    with col_v:
        ...
        # Download
        export = pd.DataFrame([{"X": x, "Y": y, "T_Max": round(max(temp_surf),2)} for x in range(50) for y in range(50)])
        towrite = BytesIO()
        export.to_excel(towrite, index=False, engine='xlsxwriter')
        st.download_button("📥 Planilha", towrite.getvalue(), f"Nayara_Simulacao.xlsx", use_container_width=True)

    with st.expander("📖 Metodologia Científica"):
        st.write(f"Modelo calibrado para Fortaleza. Emissividade: {emissividade} ({material}). Profundidade estimada estilo ENVI-met.")
